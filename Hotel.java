package travel;

import exceptions.InvalidAccommodationDataException;

public class Hotel extends Accommodation {

// Attribute
	private int starRating;
	
// Default Constructor
	public Hotel() {
		super();
		this.starRating = 1;
	}

// Parameterized Constructor
	public Hotel(String accommodationName, String location, double pricePerNight, int starRating)
		throws InvalidAccommodationDataException {
		
		super(accommodationName, location, pricePerNight); //Always first line
		
		if(starRating < 1 || starRating > 5) {
			throw new InvalidAccommodationDataException("Hotel rating is invalid.");
		}
		this.starRating = starRating;
		
	}

// Copy Construction
	public Hotel(Hotel other) {
		super(other);
		this.starRating = other.starRating;
	}

// Getters and Setters
	public int getStarRating() {
		return starRating;
	}
	public void setStarRating(int starRating) 
		throws InvalidAccommodationDataException {
		
		if(starRating < 1 || starRating > 5) {
			throw new InvalidAccommodationDataException("Hotel star rating must be between 1 and 5."); 
		}
		this.starRating = starRating;
		
	}
	
// toString() method
	@Override
	public String toString() {
		return super.toString() + "\nStar Rating: " + starRating;
	}
	
// equals() method
	 @Override
	    public boolean equals(Object obj) {
	        if (!super.equals(obj))
	            return false;

	        Hotel other = (Hotel) obj; // Casting

	      //Compare child Hotel attribute
	        return this.starRating == other.starRating;
	    }
// calculateCost() method (service fee of 10%) (Polymorphic)
	@Override
	public double calculateCost(int numberOfDays) {
		return super.calculateCost(numberOfDays) *1.1;
	}

}
